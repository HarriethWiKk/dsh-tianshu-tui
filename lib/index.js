...
function mountId(preferredId) {
	return preferredId === void 0 || preferredId === "" ? void 0 : preferredId;
}
/**
...
async function joinPreset(input) {
	...
	return {
		skipped: false,
		id: (await facet.mount(agentCtx, mountId(input.preferredId))).id
	};
}
	...
	constructor(options) {
		...
		for (const command of createBuiltinCommands({
			...
			toggleWorkflowPanel: () => {
				...
				if (this.workflowPanelVisible && scopedService(this.ctx, this.activeSessionId, "workflowEngine") === void 0) this.echoWarn("⚠ workflow 引擎不可用（未装配 workflow 插件），面板无运行数据");
				this.prefs.panels = {
					...this.prefs.panels,
