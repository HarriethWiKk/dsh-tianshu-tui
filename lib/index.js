...
function toPaletteEntries(commands) {
	return commands.map((c) => ({
		...
		group: PALETTE_COMMAND_GROUPS[c.name] ?? "其他",
...
function formatPromptFooter(input, theme) {
	...
	let segs = input.approvalPending === true ? [
		...
		"a 放行",
		"esc 取消"
	] : input.inspectOpen === true ? ["esc 关闭", "/ 命令"] : [footerTipForIndex(input.tipIndex ?? footerTipIndex())];
	for (;;) {
...
var TuiApp = class {
	...
	*  随会话卸载复位）。 */
	...
	delegationSurface = new DelegationSurfaceController({
		...
		onRunFinished: (done) => {
			...
			if (!subagentNotifySuppressed(this.workflowSurface.runningCount)) notifyOs({
				title: "dsh · 子代理完成",
				body: done.label
			}, this.prefs);
			this.renderBatcher.schedule();
	...
	*/
	atomicScrollbackWrite(writeScrollback) {
	...
	*/
	...
	renderLive() {
		...
		const rightSegments = bottomMetrics === null ? dirtySeg === null ? void 0 : [apiSeg, dirtySeg] : [
