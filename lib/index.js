...
function truncateTo$6(text, columns) {
	let out = "";
	for (const ch of text) {
...
function formatTurnSummary(input, theme) {
	...
	for (;;) {
		...
		if (list.length <= 1) return [dim(truncateTo$6(joined, width))];
		list = dropTail(list);
	}
}
//#endregion
	...
	constructor(options) {
		...
		this.slash.register({
			...
			run: () => {
				...
				if (this.statusPanelVisible && this.ctx.reflect.get("sessionProjections", false) === void 0) this.echoWarn("⚠ sessionProjections 服务不可用（未装配 session-projection 插件），状态面板无数据");
				this.renderBatcher.schedule();
			}
		});
		this.slash.register({
	...
	*/
	async handleCtrlV() {
		...
		if (text) {
			this.inputLine.insertText(text);
			this.flushLiveRender();
		} else this.echoWarn("⚠ 剪贴板读图不可用（需 osascript / wl-paste / xclip / PowerShell）");
	}
	/**
