...
function formatPromptFooter(input, theme) {
	...
	let segs = input.approvalPending === true ? [...input.approvalHints ?? DEFAULT_APPROVAL_HINTS] : input.inspectOpen === true ? [...input.inspectHints ?? DEFAULT_INSPECT_HINTS] : [footerTipForIndex(input.tipIndex ?? footerTipIndex())];
	for (;;) {
...
var TuiApp = class {
	...
	*/
	...
	renderLive() {
		...
		if (exitArmedSince !== 0) {
			if (Date.now() - exitArmedSince >= 2e3) this.actions.confirmDisarm("app.interrupt");
