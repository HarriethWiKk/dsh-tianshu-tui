...
const SKIP_UPDATE_ENV = "DSH_TUI_SKIP_UPDATE";
...
function findProfileDir(startDir) {
	...
	for (let i = 0; i < 16; i++) {
		...
		if (pkg !== void 0 && pkg.name !== TUI_PACKAGE && (pkg.dsh?.profile !== void 0 || pkg.dependencies?.[TUI_PACKAGE] !== void 0)) return dir;
...
function readOwnVersion(startDir) {
	...
	for (let i = 0; i < 8; i++) {
		const pkg = readJson(join(dir, "package.json"));
		if (pkg?.name === TUI_PACKAGE && typeof pkg.version === "string") return pkg.version;
		const parent = dirname(dir);
		if (parent === dir) break;
		dir = parent;
	}
...
async function runSelfUpdate(opts = {}) {
	...
	try {
		const latest = opts.fetchLatest === void 0 ? await fetchNpmLatest() : await opts.fetchLatest();
