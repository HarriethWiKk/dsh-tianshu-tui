...
function projectTaskPanel(tasks, width) {
	...
	for (const task of tasks) rows.push(truncateToLiveWidth(` ${statusMark(task.status)} ${task.content}`, Math.max(1, width)));
	return rows;
}
//#endregion
...
//#endregion
//#region lib/types/adapter/session-title.js
/**
...
function sessionTitleFor(events) {
	...
	if (first !== void 0) return fallbackSessionTitle(first.text, 5, 40);
	return EMPTY_TITLE;
}
//#endregion
