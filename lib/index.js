...
function isGitRepo(exec = defaultExec()) {
	try {
...
*/
function gitBranch(exec = defaultExec()) {
	try {
		const out = exec([
			"rev-parse",
			"--abbrev-ref",
...
*/
function gitDirtyCount(exec = defaultExec()) {
	try {
...
* + mod 5。Ctrl+字母与控制码映射到既有 ctrl_* 名；带修饰的可打印键保留
* char（Alt+数字等场景由上层消费）。
*/
...
async function runImageTool(candidates, outputPath, timeoutMs = 15e3) {
	...
	for (const { bin, args } of candidates) try {
		...
		await execFileAsync$1(bin, args, {
			timeout: timeoutMs,
			windowsHide: true
		});
...
function toolArgSummary(name, input) {
	switch (name) {
		case "read_file":
		case "write_file":
		case "edit_file": return truncate$3(pathBasename(input.file_path ?? input.path), 45);
		case "bash":
 /* v8 ignore next -- split('\n') 恒返回非空数组，[0] 恒存在；noUncheckedIndexedAccess 收窄防御 */
		return truncate$3(textOf(input.command).split("\n")[0] ?? "", 55);
		case "run_code": return truncate$3((textOf(input.code).split("\n")[0] ?? "") || textOf(input.description), 55);
		case "grep":
		case "glob":
		case "semantic_search": return truncate$3(textOf(input.pattern), 35);
		case "delegate_task": return truncate$3(textOf(input.objective), 50);
		case "delegate_batch": return `${Array.isArray(input.tasks) ? input.tasks.length : "?"} tasks`;
		case "web_fetch": return truncate$3(textOf(input.url), 50);
		default: return "";
	}
}
/**
...
function headerText(active, elapsedMs, lineCount) {
	...
	const elapsed = elapsedMs === void 0 ? "" : ` (${formatElapsed$2(elapsedMs)})`;
...
const KEYMAP_ENTRIES = [
	...
	{
		keys: "Ctrl+C",
		action: "打断当前回合（空闲双击退出）"
	},
	{
		keys: "Shift+Tab",
		action: "模式循环 normal→plan→always-approve"
	},
	{
		keys: "Tab",
...
/** 截断超长文本（保留头部 + 尾部 + 标记）。 */
function truncate$2(text, cap) {
	if (text.length <= cap) return text;
	return `${text.slice(0, cap)}\n…+${text.length - cap} 字符`;
}
/** 渲染一条工具结果消息（ToolResultMessage 的 content 是 ToolResultBlock 元组）。 */
function renderToolResult(message) {
	return truncate$2(message.content.flatMap((block) => block.type === "tool-result" ? block.content : []).filter((block) => block.type === "text").map((block) => block.text).join(""), TOOL_RESULT_CAP);
}
/**
...
function projectQuestion(item, width) {
	...
	if (item.header !== void 0) rows.push(truncateByWidth$4(`── ${item.header} ──`, width));
	const intent = item.intent;
	if (intent?.kind === "plan-review") {
		rows.push(truncateByWidth$4(`${g.plan} ${item.question}`, width));
	...
	rows.push(truncateByWidth$4(`${g.question} ${item.question}${multiMark}`, width));
	if (item.detail !== void 0) rows.push(...projectDetail(item.detail, width));
...
function projectDetail(detail, width) {
	return detail.split(/\r?\n/).map((line) => truncateByWidth$4(`  ${line}`, width));
}
...
function projectPlanOptions(options, approve, width) {
	...
	options.forEach((opt, i) => {
		...
		const g = questionGlyphs();
		const cut = truncateByWidth$4(`  ${isApprove ? g.approve : g.reject} ${i + 1}. ${opt.label}`, width);
...
function projectPlanKeyHints(item, width) {
	...
	return [truncateByWidth$4(`  ${hints.join("  ")}`, width)];
}
...
function projectOptionList(options, width) {
	...
	const rows = [];
	options.forEach((opt, i) => {
		rows.push(truncateByWidth$4(`  ${i + 1}. ${opt.label}`, width));
		if (opt.description !== void 0) rows.push(truncateByWidth$4(`    ${opt.description}`, width));
	});
	return rows;
}
...
function summaryLine(todos, width) {
	const counts = {
		completed: 0,
		in_progress: 0,
		pending: 0
	...
	let line = `${TITLE$4} ✓${counts.completed} ⏳${counts.in_progress} □${counts.pending}`;
	const current = todos.find((todo) => todo.status === "in_progress");
	if (current !== void 0) line += ` · ${current.content}`;
...
function projectTodosPanel(todos, opts) {
	...
	const capacity = Math.max(2, opts.maxRows ?? DEFAULT_MAX_ROWS) - 1;
	const visible = todos.length <= capacity ? todos : todos.slice(0, capacity - 1);
	for (const todo of visible) rows.push(truncateToLiveWidth(` ${statusMark(todo.status)} ${todo.content}`, width));
...
function projectGoalSection(goal, width) {
	...
	rows.push(truncateByWidth$3(`${GOAL_TITLE} · ${label.text}`, width));
	rows.push(truncateByWidth$3(goal.goal.objective, width));
	rows.push(truncateByWidth$3(`↻ 轮次 ${goal.roundsStarted}/${goal.goal.maxGoalRounds}`, width));
	if (goal.goal.phase === "blocked" && goal.goal.blockedReason !== void 0) rows.push(truncateByWidth$3(`🚧 ${goal.goal.blockedReason.message}`, width));
	return rows;
}
...
function projectPlanSection(plan, width) {
	...
	return [truncateByWidth$3(`${PLAN_TITLE} · ${mode}${pending}`, width)];
}
...
function projectSessionSection(totals, width) {
	...
	return [truncateByWidth$3(parts.join(" · "), width)];
}
/** 按显示宽度截断字符串（仅发生截断时尾部补 …；极端窄宽退化为 …）。 */
...
function renderEntry(entry, identities, timings, width) {
	...
	if (entry.kind === "diagnostic") return truncateToLiveWidth(`${indent}⚠ ${reasonLabel(entry.reason)} ${shortHash(entry.id)}`, width);
...
function projectLogRows(view, width) {
	...
	return logs.map((line) => truncateByWidth$2(`  ⤷ ${line}`, width));
}
/**
	...
	return [
		{
			...
			run: ({ text, echo }) => {
				...
				if (name === "export" || name.startsWith("export ")) {
					echo(deps.exportTheme(name.slice(6).trim() || void 0));
					return;
				}
				if (setTheme(name)) {
					...
					deps.onThemeChanged?.();
					echo(`主题已切换: ${name}`);
				} else echo(`未知主题: ${name}。可用: ${THEME_NAMES.join(", ")} / custom:<name>`);
			}
		},
		{
			name: "session",
			...
			run: async ({ text, echo, ctx }) => {
				...
				if (target === "") {
					...
					for (const preset of presets) {
						...
						const desc = preset.description === void 0 || preset.description === "" ? "" : ` — ${preset.description}`;
						echo(` ${mark} ${name} (${preset.id})${desc}`);
					}
					let currentLine = current === void 0 ? "当前: 未装配（host 默认）" : `当前: ${current}`;
					if (agent !== null) {
		...
		},
		{
			...
			run: ({ text, echo }) => {
				...
				for (const command of all) echo(`  /${command.name}${command.argsHint === void 0 ? "" : ` ${command.argsHint}`} — ${command.description}`);
				echo("快捷键见 Ctrl+. 键位表");
			}
		},
		{
...
* @returns 渲染行数组（含 ANSI）。
*/
function renderPicker(state, items, width, height, theme) {
	...
	if (items.length === 0) lines.push(color("（无选项）", theme.muted));
		...
		for (let i = 0; i < window.length; i++) {
			...
			const clipped = truncate$1(`${isSel ? "▶ " : "  "}${item.label}${marker}`, width);
...
function truncate$1(text, width) {
	let out = "";
	for (const ch of text) {
...
var PickerController = class {
	...
	*/
	open(title, items, commit, selectedIndex, hooks) {
		...
		if (selectedIndex !== void 0 && selectedIndex > 0) this.state = applyPickerEvent(this.state, {
			type: "move",
			delta: selectedIndex,
	...
	*/
	commit() {
		...
		this.onPreview = null;
		this.state = applyPickerEvent(this.state, { type: "close" });
		if (item !== void 0 && cb !== null) cb(item);
	}
	/**
...
function formatBrandWelcome(input, theme) {
	...
	const brand = truncateTo$4(input.brand ?? "dsh-tianshu-tui", input.width);
	const subtitle = truncateTo$4(`${input.subtitle ?? "DeepSeek Harness"}${input.version === void 0 ? "" : ` · v${input.version}`}`, input.width);
...
function formatWelcomeTips(input, theme) {
	...
	for (const item of items) {
		...
		const truncated = truncateTo$4(body, budget);
		if (item.available === false) {
			rows.push(color(truncated, theme.muted));
			continue;
		}
		const hintPart = color(hintText, theme.secondary);
		const labelPart = color(`  ${truncateTo$4(item.label, Math.max(0, budget - hintCol - 2))}`, theme.muted);
...
function formatTopBar(input, theme) {
	...
	const ellipsis = "…";
	return [color(`${truncateTo$3(base, Math.max(1, width - displayWidth(ellipsis)))}${ellipsis}`, theme.secondary)];
}
//#endregion
...
function formatTurnStatus(input, theme) {
	...
	if (width !== void 0 && width > 0) text = truncateTo$2(text, width);
...
function formatSlashMenu(input, theme) {
	...
	if (width < 4) return [color(truncateTo$1(ascii ? "> " : "❯ ", width), theme.muted)];
	...
	visible.forEach((item, i) => {
		...
		const labelTrimmed = truncateTo$1(labelTexts[i] ?? "", labelW);
		const pad = Math.max(0, labelW - displayWidth(labelTrimmed));
		const descW = width - displayWidth(prefix) - labelW - 2;
		const desc = descW >= 2 ? truncateTo$1(item.description, descW) : "";
	...
	if (total > maxRows) out.push(color(truncateTo$1(`  ${ascii ? "^v" : "↑↓"} 还有 ${total - maxRows} 项`, width), theme.muted));
	return out;
}
//#endregion
...
function formatSubagentRunning(input, theme) {
	return [color(truncateTo(`${input.ascii === true ? "*" : brailleSpinnerFrame(input.tick ?? 0)} 子代理 ${input.label}`, input.width), theme.primary)];
}
/**
...
function formatSubagentDone(input, theme) {
	...
	return `${color(mark, markColor)}${color(truncateTo(text.slice(1), Math.max(0, width - displayWidth(mark))), theme.muted)}`;
}
//#endregion
...
var TuiApp = class {
	...
	constructor(options) {
		...
		this.inputLine = new InputLine({
			...
			onImagesChange: (images) => {
				this.refreshAttachmentPreview(images);
			},
			onChange: (value) => {
				this.syncSlashHints();
		...
		for (const command of createBuiltinCommands({
			...
			toggleTaskPanel: () => {
				...
				if (this.taskPanelVisible && this.ctx.reflect.get("sessionProjections", false) === void 0) this.echoWarn("⚠ sessionProjections 服务不可用（未装配 session-projection 插件），任务窗格无数据");
				this.renderBatcher.schedule();
			},
			toggleSubagentsPanel: () => {
				this.subagentsPanelVisible = !this.subagentsPanelVisible;
				if (this.subagentsPanelVisible && this.ctx.reflect.get("subagents", false) === void 0) this.echoWarn("⚠ subagents 服务不可用（未装配 subagent 插件），委派树面板无数据");
				this.prefs.panels = {
					...this.prefs.panels,
			...
			toggleWorkflowPanel: () => {
				...
				if (this.workflowPanelVisible && this.ctx.reflect.get("workflowEngine", false) === void 0) this.echoWarn("⚠ workflow 引擎不可用（未装配 workflow 插件），面板无运行数据");
				this.prefs.panels = {
					...this.prefs.panels,
		...
		this.slash.register({
			...
			run: () => {
				...
				if (this.statusPanelVisible && this.ctx.reflect.get("sessionProjections", false) === void 0) this.echoWarn("⚠ sessionProjections 服务不可用（未装配 session-projection 插件），目标/任务/计划投影段无数据（会话汇总段为本地投影，不受影响）");
				this.renderBatcher.schedule();
			}
		});
		this.slash.register({
			...
			run: ({ text }) => {
				...
				if (this.todosPanelVisible && this.ctx.reflect.get("sessionProjections", false) === void 0) this.echoWarn("⚠ sessionProjections 服务不可用（未装配 session-projection 插件），待办面板无数据");
				this.renderBatcher.schedule();
			}
		});
		this.slash.register({
			...
			run: async () => {
				...
				if (this.configPanelVisible) {
					...
					if (this.configProjection === null) this.echoWarn("⚠ settings/permission/credentials 服务均不可用，设置面板无数据");
				}
				this.renderBatcher.schedule();
			}
		});
		this.slash.register({
			...
			run: () => {
				...
				if (this.skillsPanelVisible) {
					if (this.ctx.reflect.get("skills", false) === void 0) this.echoWarn("⚠ skills 服务不可用（未装配 skill 插件），技能面板无数据");
	...
	*/
	async commitHalfBlockImages(images) {
	...
	*/
	async refreshAttachmentPreview(images) {
		...
		const preview = await renderHalfBlockPreview(last, {
			maxCols: Math.max(8, Math.min(30, this.stdout.columns - 6)),
			maxRows: 10,
			background: this.previewBackground()
		});
		if (epoch !== this.attachmentPreviewEpoch) return;
		this.attachmentPreview = preview === null ? null : {
			dataUrl: last,
			lines: preview.lines
		};
		this.flushLiveRender();
	...
	*/
	ensureLspBridge() {
		...
		const lspService = this.ctx.reflect.get("lsp", false);
		let source;
		if (lspService !== void 0) {
			if (typeof lspService.getDiagnostics === "function") source = lspService;
			else if (typeof lspService.query === "function") source = officialLspSource(lspService, cwd);
		}
	...
	*/
	async openModelPicker() {
		...
		const current = this.ctx.agentDefaultModel?.currentSelection();
		const currentKey = current === void 0 ? null : `${current.provider}/${current.model}`;
		const items = [];
		let selectedIndex = 0;
		...
		picker.open("选择模型", items, (item) => {
			...
			this.commitToScrollback({
				text: `模型已切换: ${selection.provider}/${selection.model}${hot ? "（当前会话与默认均生效）" : "（默认生效）"}`,
				trailingNewline: true
			});
		}, selectedIndex);
		overlay.activate("picker");
	...
	*  实时预览：↑↓ 移动即 setTheme 生效；Enter 落定；Esc/q 还原打开前主题。 */
	...
	async openSessionPicker() {
		...
		let selectedIndex = 0;
		for (const row of rows) {
			const title = sessionTitleFor(await loadHistory(this.ctx, row.id));
			const item = {
				label: `#${shortSessionLabel(row.id)} · ${title} · ${formatSessionAge(row.createdAt, now)}${row.id === active ? "（当前）" : ""}`,
				value: row.id,
	...
	*/
	mountSession(id) {
		...
		const workflowListeners = [
			...
			this.ctx.on("workflow/log", (info, message) => {
				const run = this.workflowRuns.get(info.id);
				if (run !== void 0) {
	...
	*/
	...
	async refreshConfigProjection() {
		...
		if (credentials !== void 0) await this.fillCredentials(credentials);
	}
	...
	echoWarn(text) {
		this.commitToScrollback({
			text: color(text, this.theme.warning),
			trailingNewline: true
		});
		this.flushLiveRender();
	}
	...
	*/
	commitUserPrompt(content, images) {
		...
		if (images !== void 0 && images.length > 0 && protocol === "none") this.commitHalfBlockImages(images);
	...
	*/
	...
	handleKey(key) {
		if (key.name !== "ctrl_c" && this.inputController.ctrlCPendingSince !== 0) this.inputController.ctrlCPendingSince = 0;
		if (key.name !== "escape" && this.escRewindPendingSince !== 0) this.escRewindPendingSince = 0;
		...
		if (this.overlay?.activeId() === "rewind" && this.rewindOverlay !== null) {
			if (key.name === "ctrl_c" || key.name === "escape" && (this.rewindOverlay.isListPhase() || this.rewindOverlay.isDone())) {
				this.overlay.deactivate();
				return;
			}
		...
		if (key.name === "escape" && !this.inputController.slashMenu.open) {
			...
			if (!(this.vimEnabled && this.inputLine.vimMode === "normal")) {
				const now = Date.now();
		...
		if (key.name === "ctrl_c") {
			...
			if (this.liveAgent?.state.status === "running") {
				this.handleAbort();
				if (this.onExit !== void 0) this.inputController.ctrlCPendingSince = now;
				else this.inputController.ctrlCPendingSince = 0;
				this.flushLiveRender();
				return;
			}
			if (empty && this.onExit !== void 0) {
		...
		if (key.name === "backspace" && key.meta && this.inputLine.value === "" && this.inputLine.images.length > 0) {
	...
	*/
	...
	renderLive() {
		...
		if (this.statusPanelVisible) for (const line of renderStatusPanel(snapshot)) lines.push({ text: line });
		...
		if (ctrlCPendingSince !== 0) {
			if (Date.now() - ctrlCPendingSince >= InputController.EXIT_WINDOW_MS) this.inputController.ctrlCPendingSince = 0;
			else lines.push({ text: color("再按 Ctrl+C 退出 · Ctrl+Q 立即退出", theme.muted) });
		}
		...
		if (this.attachmentPreview !== null) for (const line of this.attachmentPreview.lines) lines.push({ text: line });
		...
		const rightSegments = bottomMetrics === null ? dirtySeg === null ? void 0 : [`API ${this.apiKeyReady ? "✓" : "✗"}`, dirtySeg] : [
			...glanceBarSegments({
				...bottomMetrics,
				width: cols,
		...
		const skipPad = (this.transcript?.view.messages ?? []).length === 0 && this.liveAgent?.state.status !== "running" && slashRows === 0 && this.dynamicRowsHighWater === 0;
		const next = nextDynamicBudget(this.dynamicRowsHighWater, dynamicRows + slashRows, ceiling + slashRows, skipPad, this.reasoningExpanded);
