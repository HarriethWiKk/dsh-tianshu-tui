...
function projectTuiSection(tui, width) {
	...
	return [truncateByWidth$2(TUI_TITLE, width), truncateByWidth$2(`  ${mark} 系统通知 · ${state}`, width)];
}
/** 宿主设置段：空数组不渲染。 */
...
var TuiApp = class {
	...
	*/
	...
	applyNotifyPref(action, echo) {
		...
		if (this.configProjection !== null) this.configProjection = {
			...
			tui: configTuiFromPrefs(this.prefs)
		};
		this.renderBatcher.schedule();
	}
	...
	*/
	...
	handleKey(key) {
		...
		if (this.configPanelVisible && this.inputLine.value === "" && (key.char === "n" || key.char === "N") && this.inputLine.vimMode === "insert") {
