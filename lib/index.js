...
*/
async function applyPrefPreset(input) {
	const { presetId, isBlank, agent, facet } = input;
	if (presetId === void 0 || presetId === "" || !isBlank || agent === null || facet == null) return { applied: false };
	try {
	...
	*/
	async newSession(reuse) {
		...
		const preset = await applyPrefPreset({
			presetId: this.prefs.preset,
			isBlank: true,
			agent: handle.agent,
			facet: this.ctx.reflect.get("agentPresets", false)
		});
