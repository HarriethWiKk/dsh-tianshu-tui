...
async function fetchNpmLatest(packageName = TUI_PACKAGE, timeoutMs = 3e3) {
...
function installNpmVersion(latest, profileDir, timeoutMs = 6e4) {
	return new Promise((resolve, reject) => {
		...
		const timer = setTimeout(() => {
			child.kill();
			reject(/* @__PURE__ */ new Error("pnpm add timed out"));
		}, timeoutMs);
		child.on("error", (err) => {
			clearTimeout(timer);
			reject(err);
		...
		child.on("exit", (code) => {
			clearTimeout(timer);
			if (code === 0) resolve();
			else reject(/* @__PURE__ */ new Error(`pnpm add exited ${code ?? "null"}`));
		});
	});
}
/**
...
async function runSelfUpdate(opts = {}) {
	...
	try {
		const plan = planSelfUpdate({
			...
			profileDir,
			installSpec,
			latest: opts.fetchLatest === void 0 ? await fetchNpmLatest() : await opts.fetchLatest()
		});
...
function formatToolCardLive(input, theme) {
	...
	while (tailLines.length < tailCount) tailLines.unshift(BODY_CONT_PREFIX);
	...
	return {
		async initialize() {
			try {
				...
				capabilities = (await rpc.request("initialize", {
...
function createBuiltinCommands(deps) {
	return [
		...
		},
		{
			...
			argsHint: "[cmd]",
			run: ({ text, echo, ctx }) => {
				const registry = ctx.tui?.commands;
				if (registry === void 0) {
					echo("⚠ 命令注册表服务不可用");
					return;
				}
...
//#endregion
//#region lib/types/format/session-tabs.js
/**
...
*/
function formatSessionTabs(tabs, width, theme) {
	if (tabs.length === 0 || width <= 0) return [];
	const segs = tabs.map((tab) => ({
		tab,
		text: `[${tab.label}${tab.current === true ? "●" : ""}]`
	}));
	for (;;) {
		const hidden = tabs.length - segs.length;
		const suffix = hidden > 0 ? ` +${hidden}` : "";
		if (displayWidth(`${segs.map((s) => s.text).join(TAB_GAP)}${suffix}`) <= width) {
			const parts = segs.map((s) => {
				return s.tab.current === true ? color(s.text, theme.primary, { bold: true }) : color(s.text, theme.dim);
			});
		...
		if (drop === -1) return [{ text: truncateToWidth(segs.map((s) => s.text).join(TAB_GAP), width) }];
		segs.splice(drop, 1);
	...
	render(width, height) {
		...
		if (this.phase === "list") {
			...
			window.forEach((m, i) => {
				...
				m.kind === "user" ? theme.userColor : theme.assistantColor;
				const line = truncateToDisplayWidth(`${mark} ${formatElapsedHuman(Date.now() - m.time)} 前 ${m.text.replace(/\n/g, " ")}`, contentWidth - 2);
...
function formatBrandWelcome(input, theme) {
	...
	const subtitle = truncateTo$4(input.subtitle ?? "DeepSeek Harness", input.width);
...
var TuiApp = class {
	...
	*/
	...
	switchToNextTab() {
		...
		const next = tabs[(tabs.findIndex((t) => t.id === active) + 1) % tabs.length];
		if (next !== void 0 && next.id !== active) this.switchSession(SessionId(next.id));
	}
	/**
	...
	*/
	...
	handleKey(key) {
		...
		if (key.meta === true && key.char >= "1" && key.char <= "9") {
			...
			if (target !== void 0 && target.id !== this.activeSessionId) this.switchSession(SessionId(target.id));
			return;
		}
		if (key.name === "ctrl_q") {
	...
	*/
	...
	renderLive() {
		...
		if (this.sessionTabs.length > 1) for (const line of formatSessionTabs(this.sessionTabs, cols, this.theme)) lines.push(line);
...
function apply(ctx, config = {}) {
	...
	ctx.inject([
		...
		runSelfUpdate({ startDir: fileURLToPath(new URL(".", import.meta.url)) }).then((result) => {
