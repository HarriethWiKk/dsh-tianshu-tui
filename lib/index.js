...
const KEYMAP_ENTRIES = [
	...
	{
		keys: "↑/↓",
		action: "输入历史（菜单打开时为选择；运行中排队时 ↑ 取回队首）"
	},
	{
	...
	flushSubmitQueue(reason) {
		...
		for (const item of items) {
			...
			this.controls?.followup(item.text, item.images).catch((err) => {
				this.commitToScrollback({
					text: `⚠ 排队消息发送失败: ${err instanceof Error ? err.message : String(err)}`,
					trailingNewline: true
				});
			});
		}
	...
	*/
	...
	handleKey(key) {
		...
		if (this.overlay?.activeId() === "scroll" && this.scrollPager !== null) {
			if (this.scrollPager.handleKey(key.name, key.char) === "close") this.overlay.deactivate();
			else this.overlay.rerender();
			return;
		}
