...
var TuiApp = class {
	...
	*  实时预览：↑↓ 移动即 setTheme 生效；Enter 落定；Esc/q 还原打开前主题。 */
	...
	async openSessionPicker() {
		...
		picker.open("选择会话", items, (item) => {
			this.switchSession(SessionId(item.value));
		}, selectedIndex);
		overlay.activate("picker");
	}
	/**
	...
	*/
	...
	renderLive() {
		...
		const skipPad = (this.transcript?.view.messages ?? []).length === 0 && this.liveAgent?.state.status !== "running";
		const next = nextDynamicBudget(this.dynamicRowsHighWater, dynamicRows, ceiling, skipPad, this.reasoningExpanded);
