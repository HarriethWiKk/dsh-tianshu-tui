...
function projectSettingsSection(settings, width) {
	...
	if (settings.length === 0) {
		rows.push(truncateByWidth$2(EMPTY_SETTINGS, width));
		return rows;
	}
...
function projectCredentialsSection(credentials, width) {
	...
	if (credentials.length === 0) {
		rows.push(truncateByWidth$2(EMPTY_CREDENTIALS, width));
		return rows;
	}
...
function projectPlanOptions(options, approve, width) {
	...
	options.forEach((opt, i) => {
		...
		const g = questionGlyphs();
		const cut = truncateByWidth(`  ${isApprove ? g.approve : g.reject} ${i + 1}. ${opt.label}`, width);
...
var TuiApp = class {
	...
	*  null = subagents 服务缺失/未预取 → 面板降级不可用）。 */
	...
	childProgress = /* @__PURE__ */ new Map();
	externalRuns = [];
	activityBandEnabled;
	activityBandMaxRows;
			...
			this.projectionDisposer = projections.onChanged((s, key, value) => {
				if (s.id !== id) {
					if (noteForeignProjection({
						...
						subagentRuns: this.subagentRuns.values(),
	...
	*/
	...
	renderLive() {
		...
		for (const line of renderActivitySection({
			enabled: this.activityBandEnabled,
