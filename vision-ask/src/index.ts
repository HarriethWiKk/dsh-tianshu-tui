/**
 * @deepseek-ai/dsh-vision-ask — vision co-pilot plugin for the dsh harness.
 *
 ...
 * durable attachment references are registered under short ids; the model can
 * then re-interrogate any retained image via `ask_image` without the user
 * re-sending it.
 *
 * The plugin registers its own llm provider route with an OpenAI-compatible
 * vision adapter (the baseline's llm-deepseek wire route is text-only), so
 * description calls ride the standard `ctx.llm.stream` path — retry/error/
 * cancel semantics preserved, description text lands in the tool result and
 * the session log (Model-visible ⟺ logged).
 *
 * @module @deepseek-ai/dsh-vision-ask
 */
...
/** Extract image attachment refs from one user message (image blocks only). */
function imageRefsFromUserMessage(event: Extract<SessionEvent, { type: 'user/message' }>): readonly unknown[] {
  return event.data.content.filter(block => block.type === 'image').map(block => (block as { attachment: unknown }).attachment)
}
...
export function apply(ctx: Context, config: Config): void {
  ...
  const registryFor = (sessionId: SessionId): ImageRegistry => {
    ...
    if (registry === undefined) {
      registry = new ImageRegistry({
        maxImages: config.registryMaxImages,
  ...
  // 注册监听：user/message 事件里的图片附件引用进注册表（所有入口覆盖）。
  ctx.on('session/event', (session, event) => {
    if (event.type !== 'user/message') return
    const refs = imageRefsFromUserMessage(event)
    if (refs.length === 0) return
    // 类型收窄：attachment 是 ImageAttachmentRef（事件边界已由会话校验）。
    registryFor(session.id).register(refs as Parameters<ImageRegistry['register']>[0])
  })

  // 视觉 adapter 注册：provider route 独立于主控，携带 image 序列化能力。
  const disposeAdapter = ctx.llm.registerAdapter([config.provider], new VisionAdapter(ctx, {
  ...
  const disposeTool = registerAskImageTool(ctx, {
    ...
    maxTokens: config.maxTokens,
    primarySupportsVision: config.primarySupportsVision,
  })
